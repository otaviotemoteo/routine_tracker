...
// One activity, as a compact row.
//
// It reports four things and offers two actions, in a single band rather than
// the stacked card this used to be. The density is the point: a list of
// activities is something you scan to find the one you want to change, and a
// screen where five activities meant five scrolls made you hunt.
//
...
export function HabitRow({
  ...
  // The measure, as a chip. For a counted or timed activity the user's own unit
  ...
  const target =
    activity.target !== null
      ? format(copy.rowTarget, { n: activity.target, unit: activity.unit ?? "" }).trim()
      : copy.rowNoTarget;
    ...
    <li className="border-2 border-forest rounded-card bg-white shadow-hard">
      ...
      <div className="min-h-[72px] flex items-center gap-3 flex-wrap px-3 py-2.5">
        ...
        <div className="min-w-0 flex-1 flex flex-col gap-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {/* A badge earns its place only when it says something: an
                ...
                worth distinguishing, and a hand-written activity needs neither. */}
            {showSource && activity.source === "ai_suggested" && (
              <span className="shrink-0 text-xs font-semibold px-2 py-0.5 rounded-full border-2 border-forest bg-straw">
                {copy.suggested}
              </span>
            )}
            {showSource && activity.source === "ai_edited" && (
            ...
            {activity.minimalAction && (
              <>
                ...
                <span className="min-w-0 truncate">
                  {format(copy.rowMinimal, { action: activity.minimalAction })}
                </span>
              </>
            )}
          </p>
        ...
        <div className="shrink-0 flex items-center gap-2.5 ml-auto">
          {/* Never rendered for an activity with no run going. UX_PRINCIPLES:
          ...
          <div className="flex gap-1.5">
            ...
            <button
              ...
              aria-label={`${copy.remove} ${activity.name}`}
              className={iconButton}
            >
