...
5. **Guided daily flow (v2.1).** Detailed logging happens at `/day?step=<slug>` — one habit per step, prefilled from the user's configured goals ("Did you do Workout today? · Chest + triceps · Bench press 4×8"), with the same progress/skip mechanics as onboarding. Each step's Save writes that habit's `details` (validated) **and** flips `done` in one `PATCH /api/checks/:id`, then advances. On Today, the "Complete daily" button starts the flow and each card deep-links into its own step; the card's corner box stays a quick-toggle (`done` only, details preserved) for rushed days, and shows a one-line badge (e.g. "+23 p", "2/2"). The spine never depends on details being filled. (Supersedes the earlier per-card modal sheet, whose per-habit form bodies in `src/components/sheets/` are now the flow's step bodies.)
